...
**IMPORTANT**: The webhook endpoint is automatically created by `stripe-replit-sync`. 

Your webhook URL (check backend logs for current URL):
```
https://4d221108-5263-4078-b2f0-4fd61a8432c2-00-3hpd1signmosk.kirk.replit.dev/api/stripe/webhook
```

The webhook is already configured in Stripe with all events (`*`). You can verify this in:
- Stripe Dashboard → Developers → Webhooks
- Look for the webhook with description: "Managed webhook for Mito subscription sync"

**No manual configuration needed** - the webhook is managed automatically!
